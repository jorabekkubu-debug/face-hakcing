...
def split_zip(input_zip, output_dir, num_parts):
    ...
    print(f"Bolaklarga ajratish: {num_parts} ta\n")

    if total == 0:
        print("XATO: ZIP ichida video topilmadi!")
        return
    ...
    with zipfile.ZipFile(input_zip, 'r') as src:
        for part_idx in range(num_parts):
            ...
            print(f"[{part_idx+1}/{num_parts}] Yaratilmoqda: {len(part_videos)} ta video...")
            ...
            print(f"   OK -> {out_path}  ({size_mb:.0f} MB, {len(part_videos)} video)")
